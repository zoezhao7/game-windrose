...
SIDEBAR_CATS = [
    {
        ...
        "children": [
            {"label": "Melee Weapons", "count": 43, "href": "/database/weapons/melee/"},
            {"label": "Ranged Weapons", "count": 19, "href": "/database/weapons/ranged/"},
            {"label": "Tools", "count": 8, "href": "/database/weapons/tools/"},
            {"label": "Ammo", "count": 7, "href": "/database/weapons/ammo/"},
        ]
    },
    {
        ...
        "children": [
            {"label": "Ship Weapons", "count": 29, "href": "/database/ships/ship-weapons/"},
            {"label": "Hull Modules", "count": 10, "href": "/database/ships/hull-modules/"},
            {"label": "Combat Orders", "count": 5, "href": "/database/ships/combat-orders/"},
        ]
    },
    {
        ...
        "children": [
            {"label": "Resources", "count": 135, "href": "/database/resources/resources/"},
            {"label": "Metals", "count": 3, "href": "/database/resources/metals/"},
        ]
    },
    {
        ...
        "children": [
            {"label": "Food", "count": 50, "href": "/database/consumables/food/"},
            {"label": "Alchemy", "count": 11, "href": "/database/consumables/alchemy/"},
            {"label": "Medicine", "count": 4, "href": "/database/consumables/medicine/"},
        ]
    },
    {
        ...
        "children": [
            {"label": "Armor", "count": 41, "href": "/database/equipment/armor/"},
            {"label": "Rings", "count": 36, "href": "/database/equipment/rings/"},
            {"label": "Necklaces", "count": 24, "href": "/database/equipment/necklaces/"},
            {"label": "Backpacks", "count": 5, "href": "/database/equipment/backpacks/"},
        ]
    },
    {
        ...
        "children": [
            {"label": "Misc", "count": 514, "href": "/database/misc/misc/"},
            {"label": "Default", "count": 6, "href": "/database/misc/default/"},
        ]
    },
]
...
def sidebar_html(active_key="all"):
    """
    ...
    lines.append('  <div class="db-sidebar-title">Categories</div>')
    lines.append('  <ul class="db-nav">')

    # 概述
    ov_cls = ' active' if active_key == "overview" else ""
    lines.append(f'    <li><a href="/database/" class="db-nav-top{ov_cls}">Overview</a></li>')

    # 所有商品
    all_cls = ' active' if active_key == "all" else ""
    lines.append(f'    <li><a href="/database/" class="db-nav-top{all_cls}">All Items <span class="db-count">{TOTAL_ITEMS}</span></a></li>')
    ...
    for cat in SIDEBAR_CATS:
        ...
        lines.append(f'        <span class="db-cat-label">{cat["label"]}</span>')
        lines.append(f'        <span class="db-count">{cat["count"]}</span>')
        lines.append(f'      </div>')
        lines.append(f'      <ul class="db-sub{collapsed}" id="sub-{cat["key"]}">')
        lines.append(f'        <li><a href="{cat["view_all"]}" class="db-sub-viewall">{cat["view_all_label"]}</a></li>')
        for child in cat["children"]:
            lines.append(f'        <li><a href="{child["href"]}" class="db-sub-item">{child["label"]} <span class="db-count">{child["count"]}</span></a></li>')
        lines.append(f'      </ul>')
        lines.append(f'    </li>')
...
# ── 页面模板 ──────────────────────────────────────────────
def make_page(title, meta_desc, breadcrumb, active_key, heading, desc, cards_html, guide_link="", css_depth=2):
    guide_banner = ""
    if guide_link:
        guide_banner = f'<div class="db-guide-link">📖 <a href="{guide_link}">View the full strategy guide for this category →</a></div>'
    css_prefix = "../" * css_depth
    db_css_prefix = "../" * (css_depth - 1)
...
<link rel="stylesheet" href="{db_css_prefix}db-style.css">
</head>
<body>
{header_html("database")}

<div class="db-layout">
{sidebar_html(active_key)}
<main class="db-main">
    <div class="db-breadcrumbs"><span>Database</span> / {breadcrumb}</div>
    <div class="db-header-meta">WINDROSE GUIDES • DATABASE</div>
    <h1>{heading}</h1>
    <p class="db-desc">{desc}</p>
    {guide_banner}
    <div class="db-toolbar">
        <input type="text" class="db-search-input" placeholder="Search by name..." id="dbSearch" onkeyup="filterCards()">
        <div class="db-toolbar-meta">Showing {card_count} items
            <label>Sort <select class="db-sort-select" onchange="sortCards(this.value)"><option value="name">Name</option><option value="rarity">Rarity</option></select></label>
        </div>
    </div>
